...
public class MovieInfoService {
    ...
    @HystrixCommand(fallbackMethod = "getFallbackRatedMovieDetails",
            threadPoolKey = "movieInfoPool",
            threadPoolProperties = {
                    @HystrixProperty(name = "coreSize", value = "20"),  // Size of thread pool
                    @HystrixProperty(name = "maxQueueSize", value = "10")   // Waiting in thread queue
    ...
    public RatedMovieDetails getRatedMovieDetails(RatedMovie ratedMovie) {
        String movieDetailsUrl = "http://movie-info-service/movies/" + ratedMovie.getMovieId();
        Movie movie = this.restTemplate.getForObject(movieDetailsUrl, Movie.class);
        return new RatedMovieDetails(movie.getName(), movie.getDescription(), ratedMovie.getAverageRating());
    }

    public RatedMovieDetails getFallbackRatedMovieDetails(RatedMovie ratedMovie) {
        return new RatedMovieDetails("Movie name not found", "", ratedMovie.getAverageRating());
    }
}
